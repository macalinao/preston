...
function Model(restifier, model) {
  ...
  model.schema.eachPath(function(pathName, path) {
    ...
    if (path.options.id && path.options.unique) {
      id = pathName;
    }
  });
  this.id = id || '_id';
...
Model.prototype.submodel = function(field, model) {
  ...
  submodel.parentCorrespondsTo = type[0].correspondsTo;
  return submodel;
};
...
Model.prototype.serve = function(router) {
  ...
  router.route(uriBase)
    .get(docFetcher, routes.query(this, Mod))
    .post(docFetcher, routes.create(this, Mod));
