...
Model.prototype.serve = function(app) {
  ...
  var uriBase = this.restifier.prefix + '/' + model.collection.name;
    ...
    if (reqQuery.populate) {
      try {
        reqQuery.populate.filter(function(toPopulate) {
          if (!thiz.model.schema.paths.hasOwnProperty(toPopulate)) {
    ...
    if (reqQuery.sort) {
      try {
        Object.keys(reqQuery.sort).filter(function(toSort) {
          if (!thiz.model.schema.paths.hasOwnProperty(toSort)) {
  ...
  app.get(uriDoc, function(req, res) {
    var query = {};
