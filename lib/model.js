...
Model.prototype.serve = function(router) {
  this.serveModel(router, '/' + this.model.collection.name, this.model);
};

Model.prototype.serveModel = function(router, uriBase, Mod, modQuery, modDoc) {
  var thiz = this;
  var uriDoc = uriBase + '/:id';

  modQuery = modQuery || function(query) {
    return query;
  };

  modDoc = modDoc || function(doc) {
    return doc;
  };
  ...
  .post(function(req, res) {
    var doc = modDoc(new Mod(req.body));
  ...
  .get(function(req, res) {
    ...
    modQuery(Mod.findOne(query)).exec(function(err, doc) {
      if (err) {
