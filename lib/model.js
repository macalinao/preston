...
Model.prototype.serve = function(router) {
  ...
  var docFetcher = function(req, res, next) {
    ...
    model.findOne(query).exec(function(err, doc) {
      ...
      thiz.model.findOne(query).exec(function(err, sai) {
        ...
        if (!sai) {
          return res.error(404, Mod.modelName + ' "' + req.params.sid + '" not found.');
        }
  ...
  var middleware = [queryHelper.queryParser, docFetcher];

  router.route(uriBase)
    .get(middleware, routes.query(this, Mod))
    .post(middleware, routes.create(this, Mod));

  router.route(uriDoc)
    .get(middleware, routes.get(this, Mod))
    .put(middleware, routes.update(this, Mod))
    .patch(middleware, routes.update(this, Mod))
    .delete(middleware, routes.delete(this, Mod));
