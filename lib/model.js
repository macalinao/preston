...
function Model(restifier, model) {
  ...
  this.middlewares = {};
  this.errMiddlewares = {};
  this.transformResponse = function(json, cb) {
    return cb(null, json);
  };
 ...
 * @param {Document} doc - The document(s) to apply the transforms to.
...
Model.prototype.applyTransforms = function(req, doc, cb) {
  ...
  async.map(docs, function(doc, nextDoc) {
    async.reduce(self.transformers, doc.toObject(), function(memo, tf, next) {
      if (tf.length === 2) {
        tf(req, memo);
        next(null, memo);
      } else {
        tf(req, memo, function(oldMemo, err, memo) {
          if (memo === undefined) {
            return next(err, oldMemo);
          } else {
 ...
 * @returns {Object} The transformed document as a JavaScript object.
 */
Model.prototype.transformResponse = function(fn) {
...
Model.prototype.use = function(route, middleware) {
  ...
  if (middleware.length === 4) {
    arr = this.errMiddlewares[route] || (this.errMiddlewares[route] = []);
  } else {
    arr = this.middlewares[route] || (this.middlewares[route] = []);
  }
  arr.push(middleware);
  return this;
};
...
Model.prototype.serve = function(router) {
  ...
  var routeMiddleware = {};
  var errMiddleware = this.errMiddlewares.all || [];
  var errRouteMiddleware = {};
  ['query', 'create', 'get', 'update', 'destroy'].filter(function(route) {
    routeMiddleware[route] = middleware.slice()
      .concat(thiz.middlewares[route] || []);
    errRouteMiddleware[route] = errMiddleware.slice().concat(thiz.errMiddlewares[route] || []);
  });


  router.route(uriBase)
    .get(routeMiddleware.query, routes.query(this), errRouteMiddleware.query)
    .post(routeMiddleware.create, routes.create(this), errRouteMiddleware.create)
    .all(routes.default);

  router.route(uriDoc)
    .get(routeMiddleware.get, routes.get(this), errRouteMiddleware.get)
    .put(routeMiddleware.update, routes.update(this), errRouteMiddleware.update)
    .patch(routeMiddleware.update, routes.update(this), errRouteMiddleware.update)
    .delete(routeMiddleware.destroy, routes.destroy(this), errRouteMiddleware.destroy)
    .all(routes.default);
...
Model.prototype.printRoutes = function() {
  ...
  _.each(this.submodels, function(s) {
    s.printRoutes();
  });
};
